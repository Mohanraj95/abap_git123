 CLASS ltcl_integration_test DEFINITION FINAL FOR TESTING
    ...
    CLASS ltcl_integration_test IMPLEMENTATION.
     ...
     METHOD create_travel.
      ...
      DATA travels_in TYPE TABLE FOR CREATE zi_rap_travel_U_2045\\travel.

      travels_in = VALUE #(     ( %cid = '12'
                                  agencyid      = 070003   "Agency 070001 does exist, Agency 1 does not exist
                                customerid    = 456
                                begindate     = today
                                enddate       = today + 30
                                bookingfee    = 30
      ...
      MODIFY ENTITIES OF zi_rap_travel_u_2045
        ENTITY travel
           CREATE FIELDS (
                              agencyid
                              customerid
      ...
      SELECT * FROM zi_rap_travel_u_2045 WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
    ...
    METHOD class_setup.
      cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
          i_for_entities = VALUE #( ( i_for_entity = 'ZI_RAP_TRAVEL_U_2045' )
                                  ( i_for_entity = 'ZI_RAP_BOOKING_U_2045' ) )
                                ).
    ENDMETHOD.
